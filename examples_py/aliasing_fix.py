...
time_samples = np.linspace(0, 1, SAMPLING_FREQUENCY)
...
my_plot(time_samples, {f'signal + alias samples ({SAMPLING_FREQUENCY}Hz)':sum_sig_samples}, styles = ['C1.'], res=RES)

freqs = np.linspace(0, SAMPLING_FREQUENCY/2, int(SAMPLING_FREQUENCY/2+1))
my_plot(freqs, {'Samples dft':spectrum_samp_abs}, styles = ['C1'], stem=True, res=RES)
plt.xlabel('frequency')
plt.ylim(0,30)
plt.xlim(0, 26)

my_plot(time, {f'signal ({SIGNAL_FREQUENCY}Hz) + potential alias ({ALIAS_FREQUENCY}Hz)':sum_signal}, res=RES)
plt.legend(loc = 'lower center', ncol=1 , bbox_to_anchor=(0.5, 0.9), framealpha=0, fontsize = 7)

my_plot(time, {'Signal with alias fileterd out':filt_sine}, styles = ['C2'], res=RES)
plt.legend(loc = 'lower center', ncol=1 , bbox_to_anchor=(0.5, 0.9), framealpha=0, fontsize = 7)

freqs = np.linspace(0, 10e4/2, int(10e4/2))
my_plot(freqs, {'dft':spectrum_filt_abs}, styles = ['C2'], stem=True, res=RES)
plt.xlabel('frequency')
plt.ylim(0,60000)
plt.xlim(0, 120)
plt.xticks(np.arange(0,120, step=10))
...
my_plot(time_samples, {f'Filtered signal samples ({SAMPLING_FREQUENCY}Hz)':sum_sig_filt_samples}, styles = ['C1.'], res=RES)
plt.legend(loc = 'lower center', ncol=1 , bbox_to_anchor=(0.5, 0.9), framealpha=0, fontsize = 7)

freqs = np.linspace(0, SAMPLING_FREQUENCY/2, int(SAMPLING_FREQUENCY/2+1))
my_plot(freqs, {'Samples dft':spectrum_samp_filt_abs}, styles = ['C1'], stem=True, res=RES)
plt.xlabel('frequency')
plt.ylim(0,30)
plt.xlim(0, 26)
plt.legend(loc = 'lower center', ncol=1 , bbox_to_anchor=(0.5, 0.9), framealpha=0, fontsize = 7)